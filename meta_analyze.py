...
def extract_topic(workspace: pathlib.Path) -> str:
    """Extract the research topic from program.md's Problem Statement section."""
    program_md = workspace / "program.md"
    if program_md.exists():
        text = program_md.read_text()
        # Look for the bold topic in the Problem Statement
        import re
        match = re.search(r'\*\*(.+?)\*\*', text.split("## Problem Statement")[-1][:500]) if "## Problem Statement" in text else None
        if match and match.group(1) != "YOUR_TOPIC_HERE":
            return match.group(1)
    return "the domain specified in program.md"
...
1. **Coverage gaps**: Which major sub-topics of {topic} have NOT been covered or are only partially covered?
